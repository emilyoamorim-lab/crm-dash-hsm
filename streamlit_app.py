...
if arquivo:
    try:
        ...
        # Mapeamento de Colunas (Seu App "aprende" o layout da planilha)
        st.sidebar.subheader("Mapeamento de Colunas")
        col_bu = st.sidebar.selectbox("Produto/BU:", df.columns, index=0)
        col_periodo = st.sidebar.selectbox("Coluna de Mês/Data:", df.columns, index=3) # Geralmente a coluna 3 ou 4
        ...
        todos_meses = sorted(df[col_periodo].unique().tolist())
        meses_selecionados = st.sidebar.multiselect("Selecionar Período:", options=todos_meses, default=todos_meses)
        
        # Filtro de Unidade (Produto)
        ...
        # Aplicando os filtros na base
        df_filtrado = df[
            (df[col_bu].isin(unidades_selecionadas)) & 
            (df[col_periodo].isin(meses_selecionados))
        ].copy()

        # --- CORREÇÃO MATEMÁTICA (Decimal para Porcentagem) ---
        ...
        df_mensal = df_filtrado.groupby(col_periodo)[col_abertura].mean().reset_index()
        fig_evol = px.line(df_mensal, x=col_periodo, y=col_abertura, markers=True, 
                          title="Tendência de Performance (Média por Mês)",
                          labels={col_abertura: "Taxa de Abertura (%)"},
                          text=df_mensal[col_abertura].map('{:.1f}%'.format))
        st.plotly_chart(fig_evol, use_container_width=True)

        # --- TABELA DETALHADA ---
        st.subheader("📋 Detalhes dos Disparos")
        st.dataframe(df_filtrado[[col_periodo, col_bu, col_abertura, col_clique]].sort_values(by=col_periodo), use_container_width=True)

    except Exception as e:
        st.error(f"Erro ao processar: {e}. Verifique as colunas selecionadas.")
else:
    st.info("👋 Suba sua planilha de CRM para ver a análise histórica!")
